package com.bdb.oplbacthsemanal.persistence.model.response;
...
public class ResponseServiceImpl implements ResponseService{
    ...
    public ResponseBatch resultJob(String status, String requestUrl, String jobId, String resultJob, String messagge , Collection<StepExecution> step) {
        ...
        return new ResponseBatch(LocalDateTime.now().toString(),status,requestUrl,jobId,resultJob,possibleMistake[0] != null ?
                possibleMistake[0] : messagge);
    }
    ...
    public ResponseEntity<ResponseBatch> getResponseJob(JobExecution jobExecution, String message, HttpServletRequest urlRequest) {
        ...
        if (jobExecution.getStatus().isUnsuccessful()) {
            ...
            return new ResponseEntity<>(responseBatch(jobExecution, urlRequest, message), HttpStatus.INTERNAL_SERVER_ERROR);
        } else {
            return new ResponseEntity<>(responseBatch(jobExecution, urlRequest, null), validateStatus(jobExecution));
        }
