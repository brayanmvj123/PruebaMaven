...
@CommonsLog
@Api(value = "Modulo Batch Mensual")
public class ControllerCalendarioCofnal {

    @Autowired
    CalendarioCofnalService calendarioCofnalService;

    @ApiOperation(value = "Este servicio se encarga de traer el calendario consultando el servicio de OPLCofnal.",
    ...
    @GetMapping("carga/calendariocofnal")
    public ResponseEntity<ResponseBatch> tasasVariablesCofnalUltimoMes(HttpServletRequest urlRequest) throws JobParametersInvalidException, JobExecutionAlreadyRunningException, JobRestartException, JobInstanceAlreadyCompleteException {
        log.info("SE INICIA LA CONSULTA Y CARGUE DE INFORMACIÓN PARA LLENAR LA TABLA OPL_CAR_CALENDARCON_DOWN_TBL.");
        return calendarioCofnalService.consumeJobsCalendario(urlRequest);
    }
}
