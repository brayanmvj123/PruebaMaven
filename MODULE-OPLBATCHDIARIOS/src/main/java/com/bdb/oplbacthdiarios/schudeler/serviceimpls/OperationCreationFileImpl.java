...
public class OperationCreationFileImpl implements OperationCreationFile {
    ...
    public void creationFile(String host) throws Exception {
        ...
        List<JSONGetSalPgDiaria> salPgSemanalList = mapperReportPg.listColumnsReportPgWeeklytoJSONGetSalPgDiaria(repositorySalPg.cdtsReconcilied());
        List<Long> nroOficinasSorted = salPgSemanalList.stream().map(x -> Long.valueOf(x.getNroOficina())).sorted().distinct().collect(Collectors.toList());
        ...
        for (Long nroOficina : nroOficinasSorted) {
            ...
            List<JSONGetSalPgDiaria> jsonGetSalPgSemanaSorted = salPgSemanalList.stream().filter(x -> Long.valueOf(x.getNroOficina()).equals(nroOficina))
                        .sorted(Comparator.comparing(JSONGetSalPgDiaria::getItem)).collect(Collectors.toList());
...
//            DataOutputStream out = new DataOutputStream(new FileOutputStream("C:\\Users\\andre\\Documents"+"\\OPALO\\" + serviceFTP.obtenerFechaActual() + "\\OUTPUT\\REPORTE_OFICINA_DIARIO\\" + nameFile));
            ...
            String path = "/OPALO/"+fechaActual+"/OUTPUT/REPORTE_OFICINA_DIARIO/";
