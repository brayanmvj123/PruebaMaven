...
import {OfficeInfo} from "../../models/validate.office.model";

@Component({
  selector: 'app-liquidation-weekly',
  templateUrl: './liquidation-weekly.component.html',
  styleUrls: ['./liquidation-weekly.component.scss'],
  animations: [
...
export class LiquidationWeeklyComponent implements OnInit, OnDestroy {
  ...
  // Table
  public dataSource: LiquidationWeeklyElement[] = [];
  ...
  public expandedElement?: LiquidationWeeklyElement | null;
  ...
  ngOnInit() {
    ...
    this.dataSourcePag.paginator._intl.itemsPerPageLabel = 'Liquidación semanal por página';
    // Set user data
  ...
  public getAllUsers(): void {
    ...
    this.xlsxUserCells = [];
    this.liquidationService.getSalPgSemanal().subscribe(response => {
      this.tableLoading = false;
      response.result.forEach(item => {

        if( parseFloat(item.tipPosicion) == 1 ){          
          /* Se valida si existen CDT's diferentes a Tesoreria o Digital, para validar su estado de Conciliación y el estado de envio de reporte */
          this.tesor = this.tesor + 1;    
          if( this.envio === 0 ){
            if(Number.parseInt(item.estado) === 4 && Number.parseInt(item.enviado) === 1){
              this.enableSentConci = false;
              this.blockConcili = true;          
            }if(Number.parseInt(item.enviado) === 0){
              console.log("Existe al menos un CDT de Oficina al cual no se le envió el reporte.");
              this.envio = this.envio + 1;
              this.enableSentConci = true;
              this.blockConcili = false;
            }
          }
        }else{
          if( this.tesor != 0 ){
            console.log("Existen CDTs de tipo Oficina.");
          }else{
            if(Number.parseInt(item.estado) === 4){              
              /* Al ingresar acá se validó que el comportamiento de los CDT's de Tesoreria no se tomara 
              en cuenta frente a las validaciones de Estado (Cierre conciliación) y Enviado (Envío de informes). */
              this.enableSentConci = false;
              this.blockConcili = true;                  
            }
          }
      ...
      this.tableLoading = false;
      Uikit.notification().danger(error.error.message || 'Hubo un error inesperado al intentar obtener la lista de los registros de liquidación semanal.');
    });
  }
  ...
  public async downloadExcel(): Promise<void> {
    ...
    data.title = 'Reporte de vencimientos Liquidación Semanal';
    ...
    try {
      ...
      link.href = url;
      link.download = `Vencimientos_liquidacion_semanal_${Moment().format('YYYYMMDD')}.xlsx`;
      link.click();
        ...
        if ( val.numCdt === item[2] && val.codIsin === item[3] && val.ctaInv === item[4]){
          ...
          const dialogRef = this.dialog.open(LiquidationWeeklyDialogComponent, {
            data: {
              ...
              // Permite determinar que corresponde a Liq semanal = 2
              valLiquiOption: 2,
              buttonText: {
                cancel: 'Done'
              }
  ...
  confirmSend(): void{
    const ls: DialogListModel[] = [
      { name: 'Cerrando conciliación semanal', loading: 1 },
      { name: 'Creando reporte con los CDTs Desmaterializados por oficina.', loading: 1 },
      { name: 'Validando información de las oficinas afectadas', loading: 1 },
      { name: 'Enviando el reporte a cada oficina.', loading: 1 }
    ];
  ...
  closeConsiliation(ls: DialogListModel[], isError: { show: boolean }, isSuccess: { show: boolean }): void {
    console.log("Servicio para cerrar liquidación semanal.");
    this.liquidationService.closeConciliationWeekly().subscribe(response => {
      console.log("resp", response);
      isSuccess.show = true;
      ls[0] = { name: 'Conciliación cerrada exitosamente.', loading: 2 };
      this.liquidationService.sendEmail().subscribe(response =>{
        ls[1] = {name: 'Reportes creados.', loading: 2};
        ls[3] = {name: 'Enviando correos...', loading: 1};
        if (response.status.code === 206) {
          ls[1] = {name: 'Se envian los reportes a las oficinas afectadas, mas sin embargo hay ciertas oficinas con ' +
              'conflictos para el envio del reporte.', loading: 2};
          ls[2] = {name: 'Algun(as) oficina(s) NO cuenta(n) con la información adecuada.', loading: 3};
          this.liquidationService.validateOffice().subscribe(response => {
            if (response.status.code === 200) {
              isSuccess.show = true;
              ls[1] = {name: 'Las oficinas cuenta la información adecuada.', loading: 2};
            } else {
              isSuccess.show = false;
              isError.show = true;
              ls[2] = {name: 'Algun(as) oficina(s) NO cuenta(n) con la información adecuada.', loading: 3};
              this.openDialog();
            ...
            isError.show = true;
            ls[1] = {
              name: error.error.message || 'Hubo un error inesperado al realizar la validación de la información de ' +
                'la oficina.', loading: 3
            };
          })
        }else if (response.status.code === 200){
          ls[2] = {name: 'Oficinas validadas con exito.', loading: 2};
          ls[3] = {name: 'Las oficinas han sido informadas.', loading: 2};
        }
      }, error => {
        isError.show = true;
        ls[1] = { name: error.error.message || 'Error al crear, guardar o enviar correos a la oficina.', loading: 3 };
      })
    }, error => {
      isError.show = true;
  ...
  openDialog() {
    const dialogRef = this.dialog.open(InfoOfficeDialogComponent, {maxWidth:'45%'});
...
export interface LiquidationWeeklyElement {
  item: string;
  nroOficina: string;
  depositante: string;
  numCdt: string;
